...
const DashboardContainerLayout = ({
  children,
  router,
  ...
  const isLifeSurvey = currentPath?.includes("life-survey");

  return (
    <>
      {UserCurrentStatus === "user" && isLifeSurvey ? (
        children
      ) : (
        <div className="mx-auto p-20 flex rounded-50 max-w-[1212px] relative cusBackgroundBackdrop min-h-[777px]">
          <div className="w-[204px]">
            <div className="profile flex flex-col items-center">
              ...
              {isMounted && (
                <p className="mt-4 text-20 font-normal relative">
                  {newUserName}
                </p>
              )}
            </div>
